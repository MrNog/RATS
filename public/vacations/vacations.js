...
function officerMembers() {
  return guildRoster().filter((m) => !isAltG(m) && !/pug/i.test(m.rankName || ""));
}
function pickerList() {
  return isOfficer ? officerMembers() : PUBMEMBERS;
}
async function loadPickerMembers() {
  ...
  if (window.RatsData && RatsData.loadMembers) {
    try {
      const m = await RatsData.loadMembers();
      if (Array.isArray(m)) {
        PUBMEMBERS = m;
        return;
      }
  ...
  try {
    const r = await fetch(FB + "members.json", { cache: "no-store" });
    const o = r.ok ? await r.json() : null;
    PUBMEMBERS = Array.isArray(o) ? o : [];
  } catch (e) {
    PUBMEMBERS = [];
  }
}
...
function render() {
  ...
  if (isOfficer) {
    renderCalendar();
    if (window.RatsCal) RatsCal.enhanceAll();
  } // enhance edit-row date inputs
}

// ---- month calendar (officers only): who's away each day, overlaps as stacked chips ----
let calYear = new Date().getFullYear(),
  calMonth = new Date().getMonth(); // month 0-11
function calShift(n) {
  calMonth += n;
  if (calMonth < 0) {
    calMonth = 11;
...
function vacsOnDay(dStr) {
  return VACS.filter((v) => v.start && dStr >= v.start && dStr <= (v.end || v.start)).sort((a, b) =>
...
function renderCalendar() {
  ...
  document.getElementById("calLabel").textContent = first.toLocaleDateString(undefined, {
    month: "long",
    year: "numeric",
  });
...
async function saveEdit(key) {
  const v = VACS.find((x) => x.key === key);
  if (!v) return;
  const s = document.getElementById("edStart").value,
    e = document.getElementById("edEnd").value;
  if (!s) {
    msg("From date is required.", "#ff6b6b");
    return;
  }
  if (e && e < s) {
    msg("'To' is before 'From'.", "#ff6b6b");
    return;
  }
  editingKey = "";
  msg("⏳ Saving…", "#8a8d93");
  try {
    await RatsData.updateVacation(key, {
      name: v.name,
      class: v.class || "",
      start: s,
      end: e || "",
      note: v.note || "",
      type: v.type || "vacation",
    });
    await reload();
    msg("✅ Updated " + v.name + " — shared.", "#7CFC8A");
  } catch (err) {
    msg("❌ " + (err && err.message ? err.message : err), "#ff6b6b");
  }
}
...
function acFilter() {
  ...
  if (!all.length) {
    list.innerHTML = '<div class="none">No member list yet — ask an officer to save the roster.</div>';
  ...
  list.innerHTML = ms.length
    ? ms
        .map(
          (m) =>
            `<div class="opt" data-n="${esc(m.name)}" data-c="${esc(m.class || "")}" style="color:${CLASS_COLOR[m.class] || "#fff"}">${esc(m.name)}</div>`
        )
        .join("")
...
function vacWebhook() {
  try {
    ...
    const h = a.find((x) => /vacation/i.test(x.name || ""));
    return (h && h.url) || "";
  } catch (e) {
    return "";
  }
...
const CLASS_QUIPS = {
  Warrior: [
    "Charged off the edge of the calendar.",
    "Gone to find a bigger axe.",
    "Rage went on cooldown.",
    "Heroic Leap'd straight to the beach.",
  ],
  Paladin: [
    "Bubble-hearthed to the beach.",
    "Even the Light takes a day off.",
    "AFK, blessing the buffet.",
    "Divine Shield up, raid invites bounce off.",
  ],
  Hunter: [
    "Off feeding the pet… and himself.",
    "Misdirected all his duties to someone else.",
    "Gone hunting (the couch).",
    "Feign Death until further notice.",
  ],
  Rogue: ["Vanished. Typical.", "Stealthed out of the raid roster.", "Pick-pocketed some PTO.", "Sapped by real life."],
  Priest: [
    "Praying somewhere with better Wi-Fi.",
    "Out of mana for responsibilities.",
    "Faded — you can't see them now.",
    "Took a Spirit of Redemption nap.",
  ],
  Shaman: [
    "Reincarnated… on a sofa.",
    "Ghost Wolf'd out the door.",
    "Totems left running, owner gone.",
    "Far Sight set to 'somewhere nice'.",
  ],
  Mage: [
    "Blinked away from the raid.",
    "Conjured snacks, not attendance.",
    "Portal's down — can't make it back.",
    "Spent the week eating their own food.",
  ],
  Warlock: [
    "Sold their soul for vacation days.",
    "Summoning stone, but make it tropical.",
    "The demons get PTO too.",
    "Soulstoned, will rez after the holiday.",
  ],
  Druid: [
    "Hibernating — don't poke the bear.",
    "Travel Form: straight to bed.",
    "Rooted to the couch.",
    "Prowling the snack cupboard.",
  ],
  "Death Knight": [
    "On unholy leave.",
    "Risen from the grave, gone to brunch.",
    "Frost-bitten by the urge to rest.",
    "Army of the Dead can cover the raid.",
  ],
  DK: ["On unholy leave.", "Army of the Dead can cover the raid."],
};
const GENERIC_QUIPS = [
  "Even rats need a nap. 🧀",
  "Off finding cheese.",
  "Scurried off for a bit. 🧀",
  "Gone to gnaw on something nice.",
];
function quipFor(v) {
...
function buildPersonalEmbed(v) {
  ...
  return {
    author: { name: "RATS • Time off" },
    title: "💤 " + v.name + " is away for a bit",
...
function buildVacEmbed(v) {
  ...
  const endD = v.end || v.start,
    s = noon(v.start),
    e = noon(endD);
  const ret = new Date(endD + "T12:00:00");
  ret.setDate(ret.getDate() + 1);
  const r = Math.floor(ret.getTime() / 1000),
    days = daysBetween(v.start, v.end);
  let desc =
    "_" +
    quipFor(v) +
    "_\n\n" +
  ...
  if (v.note) desc += "\n📝 " + v.note;
  return {
    author: { name: "RATS • Vacations" },
    title: "🏖️ " + v.name + " will be away",
    color: parseInt(hex.slice(1), 16),
    description: desc,
  };
}
async function postVacEmbed(url, v) {
  await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ embeds: [buildVacEmbed(v)] }),
  });
}
...
function buildReminderEmbed(v) {
  ...
  const r = Math.floor(ret.getTime() / 1000);
  return {
    author: { name: "RATS • Vacation reminder" },
    title: "🔔 " + v.name + " starts their absence tomorrow",
    description: "⏳ Back <t:" + r + ":R>",
    color: parseInt(hex.slice(1), 16),
  };
}
async function postReminder(url, v) {
  await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ embeds: [buildReminderEmbed(v)] }),
  });
}
const tomorrowStr = () => {
  const d = new Date();
...
async function announceNew() {
  ...
  for (const v of VACS) {
    ...
    try {
      await postVacEmbed(url, v);
      const e = Object.assign({}, v);
      delete e.key;
      e.announced = true;
      await RatsData.updateVacation(v.key, e);
      v.announced = true;
    } catch (err) {
      /* leave unannounced; retry next refresh */
    }
  }
}
async function announceReminders() {
  ...
  for (const v of VACS) {
    ...
    try {
      ...
      /* leave un-reminded; retry next refresh */
    }
  }
}
async function purgeExpired() {
  ...
  for (const v of stale) {
    try {
      await RatsData.removeVacation(v.key);
    } catch (e) {}
  }
...
async function addVac() {
  ...
  if (!name || name.toLowerCase() !== typed.toLowerCase()) {
    ...
    if (m) {
      ...
      msg("Pick your character from the list.", "#ff6b6b");
      return;
    }
  }
  const start = document.getElementById("vStart").value,
  ...
  try {
    ...
    msg("❌ " + (e && e.message ? e.message : e), "#ff6b6b");
  }
}
...
async function repostVac(key) {
  ...
  const url = vacWebhook();
  if (!url) {
    msg("❌ No 'Vacations' webhook saved (Admin → Discord webhooks).", "#ff6b6b");
    return;
  }
  msg("⏳ Posting…", "#8a8d93");
  try {
    await postVacEmbed(url, v);
    msg("✅ Re-posted " + v.name + "'s alert to Discord.", "#7CFC8A");
  } catch (e) {
    msg("❌ " + (e && e.message ? e.message : e), "#ff6b6b");
  }
}
async function removeVac(key) {
  msg("⏳ Removing…", "#8a8d93");
  try {
    await RatsData.removeVacation(key);
    await reload();
    msg("✅ Removed — shared.", "#7CFC8A");
  } catch (e) {
    msg("❌ " + (e && e.message ? e.message : e), "#ff6b6b");
  }
}
...
async function boot() {
  ...
  if (isOfficer) {
    ...
    const att = document.getElementById("attLink");
