...
function realClass(m) {
  const c = m && m.className;
  if (c && CLASS_COLOR[c]) return c === "DK" ? "Death Knight" : c;
  return classFromSpec(m && m.specName) || c || "";
}
function msg(t, c) {
...
function joinDateOf(name) {
  const j = guildData().joined || {},
    n = normName(name);
...
function rosterKeys(r) {
  // Set of "toon|class" for everyone in the groups
  const set = new Set();
  (r.groups || []).forEach((g) =>
    (g.members || []).forEach((m) => {
      const nm = normName(m.name);
      if (nm) set.add(nm + "|" + realClass(m));
    })
  );
  return set;
}
...
function raidInstOf(r) {
  return window.RatsData && RatsData.raidKeyOf ? RatsData.raidKeyOf(r.desc) || "Other" : "Other";
}
...
function computeAttendance(raids) {
  ...
  function ent(name, cls) {
    const main = resolveMain(name),
      key = main.toLowerCase();
    if (!map[key]) {
      ...
      map[key] = {
        ...
        excused: new Set(),
        rankIndex: rm && rm.rankIndex != null ? rm.rankIndex : 99,
        rankName: (rm && rm.rankName) || "",
      };
  ...
  raids.forEach((r) => {
    ...
    (r.groups || []).forEach((g) =>
      (g.members || []).forEach((m) => {
        const o = ent(m.name, realClass(m)),
          key = o.name.toLowerCase();
        if (!seen.has(key)) {
          o.present++;
          seen.add(key);
        }
        o.involved.add(r.date);
        o.presentDates.add(r.date); // showed up -> always counts (even if on vacation)
        // roster members keep their MAIN's class; pugs show whatever they last raided
        if (r.date >= o.lastDate) {
          o.lastDate = r.date;
          if (!o._roster) {
            const rc = realClass(m);
            if (rc) o.cls = rc;
          }
        }
      })
    ...
    (r.noshows || []).forEach((m) => {
      ...
      if (m.vacation || onVacation(o.name, r.date)) {
        o.excused.add(r.date);
      } // 🏖️ excused -> never counts
      else {
        o.noShow++;
        o.involved.add(r.date);
      } // signed up & missed -> counts (+ red counter)
    });
  });
    ...
    guildRoster().forEach((m) => {
      ...
      if (key && !map[key])
        map[key] = {
          ...
          presentDates: new Set(),
          excused: new Set(),
          rankIndex: m.rankIndex != null ? m.rankIndex : 99,
          rankName: m.rankName || "",
        };
    });
  }
...
function renderAttendance() {
  ...
  if (!total) {
    el.innerHTML =
      '<div class="empty">No raids saved in this range yet. Build a comp and hit <b>💾 Save to history</b>.</div>';
    return;
  }
  if (!list.length) {
    el.innerHTML = '<div class="empty">No raiders match.</div>';
    return;
  }
  ...
  if (byRank && collapsedRanks === null) {
    ...
    list.forEach((r) => {
      ...
      if (/sewer/i.test(rk)) collapsedRanks.add(rk);
    });
    saveCollapsed();
  }
  ...
  list.forEach((r) => {
    ...
    const ghost = r.noShow
      ? `<span class="pill" title="Signed up but never showed">${r.noShow}&times; ghost</span>`
      : "";
    const half = r.halves
...
function setRange(b) {
  RANGE_DAYS = parseInt(b.dataset.d) || 0;
  document.querySelectorAll("#rangeSegs .seg").forEach((s) => s.classList.toggle("active", s === b));
  rerender();
}
function setRaid(b) {
  RAID = b.dataset.r;
...
let collapsedRanks = (function () {
  try {
    const a = JSON.parse(localStorage.getItem("ratsAttCollapsed") || "null");
...
function saveCollapsed() {
  try {
    localStorage.setItem("ratsAttCollapsed", JSON.stringify([...(collapsedRanks || [])]));
  } catch (e) {}
}
function toggleRank(rk) {
  if (!collapsedRanks) collapsedRanks = new Set();
  if (collapsedRanks.has(rk)) collapsedRanks.delete(rk);
  else collapsedRanks.add(rk);
  saveCollapsed();
  renderAttendance();
}
...
function runBodyHtml(r) {
  ...
  const nsRows = (r.noshows || [])
    .map((m) => {
      ...
      const img = ico
        ? `<img class="ic" src="${CDN(ico)}" onerror="this.style.visibility='hidden'">`
        : `<span class="ic"></span>`;
...
function groupCard(runs, gid) {
  ...
  const uniq = new Set();
  ordered.forEach((r) =>
    (r.groups || []).forEach((g) => (g.members || []).forEach((m) => uniq.add((m.name || "").trim().toLowerCase())))
  );
...
function editInComp(date) {
  ...
  const raid = /icc|icecrown/i.test(d)
    ...
    : /uld/i.test(d)
      ? "Ulduar"
      : /to[gc]c?|crusader/i.test(d)
        ? "ToC"
        : /ony/i.test(d)
          ? "Ony"
          : /naxx/i.test(d)
            ? "Naxx"
            : /\brs\b|ruby|halion/i.test(d)
              ? "RS"
              : "ICC";
...
function postRaidToDiscord(date) {
  ...
  const emo = (m) => {
    const id = m.specEmoteId || m.classEmoteId;
  ...
  let tanks = 0,
    ...
    dps = 0,
    total = 0;
  const fields = (r.groups || [])
    .filter((g) => (g.members || []).length)
    .map((g) => {
      (g.members || []).forEach((m) => {
        total++;
        const s = m.specName || "";
        if (tankSpecs.test(s)) tanks++;
        else if (healSpecs.test(s)) heals++;
        else dps++;
      });
      return {
        name: g.name,
        value: (g.members || []).map((m) => emo(m) + "**" + m.name + "**").join("\n") || "—",
        inline: true,
      };
    });
  const ns = r.noshows || [];
  ...
  const embed = {
    ...
    fields,
    footer: {
      text: total + " raiders   •   " + tanks + " Tank · " + heals + " Healer · " + dps + " DPS   •   FOR THE RATS 🧀",
    },
  ...
  fetch(url, {
    ...
    .then((rr) => {
      if (rr.ok) msg("✅ Posted to Discord!", "#7CFC8A");
      else msg("❌ Discord rejected it (HTTP " + rr.status + ").", "#ff6b6b");
    })
    .catch(() =>
      msg("❌ Post blocked (CORS / file:// origin). Open the tools via a local server, not file://.", "#ff6b6b")
    );
}
...
async function saveTitle(date) {
  ...
  if (!pass) {
    msg("⚠️ Changed locally — open from the index & enter the guild key to share it.", "#e0b860");
    return;
  }
  msg("⏳ Saving…", "#8a8d93");
  try {
    const res = await RatsData.saveHistory(HIST, pass);
    msg(res.mode === "firebase" ? "✅ Title updated & shared." : "✅ Saved — commit history.json via Fork.");
  } catch (e) {
...
async function deleteRaid(date) {
  ...
  if (
    ...
    return;
  if (!window.RatsData) {
    msg("❌ Data layer not loaded.", "#ff6b6b");
    return;
  }
  ...
  try {
    ...
    msg(
      res.mode === "firebase"
        ...
        : "✅ Deleted — commit the downloaded history.json via Fork to publish."
    );
  } catch (e) {
    msg("❌ Delete failed: " + e.message, "#ff6b6b");
  }
}
...
async function setOptional(date, on) {
  ...
  try {
    ...
    msg(
      res.mode === "firebase"
        ? "✅ " + (on ? "Marked optional (log-only)." : "Now counts for attendance.") + " Shared."
        : "✅ Saved — commit history.json via Fork."
    );
  } catch (e) {
    msg("❌ Save failed: " + e.message, "#ff6b6b");
  }
...
async function boot() {
  ...
  try {
    HIST = (window.RatsData ? await RatsData.loadHistory({ interactive: false }) : { raids: [] }) || { raids: [] };
  } catch (e) {
    HIST = { raids: [] };
  }
