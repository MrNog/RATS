...
const FILES = [
  {
    ...
    files: [
      {
        name: "WOTLK Phase 3 — Loot Prio Sheet",
        file: "files/WOTLK Phase 3 Loot Prio Sheet.xlsx",
        type: "XLSX",
        size: "2.3 MB",
        date: "2026-06",
...
function enc(s) {
  return encodeURI(String(s == null ? "" : s));
  ...
  tree.innerHTML = FILES.map((folder, fi) => {
    ...
    const rows = files
      .map((f) => {
        ...
        const openHref = f.open
          ? f.open
          : viewable
            ? "https://docs.google.com/viewer?url=" + encodeURIComponent(absUrl)
            : dl;
