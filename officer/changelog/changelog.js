...
function fmtDate(s) {
  ...
  if (m) return +m[1] + " " + MON[+m[2] - 1] + " " + m[3];
  return s;
}

async function fbGet(p) {
  try {
    const r = await fetch(FB + p + ".json", { cache: "no-store" });
    return r.ok ? await r.json() : null;
  } catch (e) {
    return null;
  }
}
async function fbPost(p, o) {
...
function renderCard(embed) {
  ...
  const desc = esc(embed.description)
    .replace(/\*(.+?)\*/g, "<i>$1</i>")
    .replace(/\n/g, "<br>");
...
function renderPreview() {
  document.getElementById("preview").innerHTML = renderCard(buildEmbed(draft()));
}
...
async function postToDiscord(e) {
  ...
  if (!url) {
    msg("❌ No #okanor-logs webhook saved (Admin → Discord webhooks, name it with 'log').", "#ff6b6b");
    return false;
  }
  await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ embeds: [buildEmbed(e)] }),
  });
  return true;
}
async function addEntry() {
  const e = draft();
  if (!e.items.length) {
    msg("Write at least one change (one per line).", "#ff6b6b");
    return;
  }
  ...
  try {
    ...
    if (document.getElementById("alsoPost").checked) {
      try {
        if (await postToDiscord(e)) note += " Posted to #okanor-logs.";
      } catch (err) {
        note += " (Discord post failed: " + err.message + ")";
      }
    }
...
async function repost(key) {
  const e = LOGS.find((x) => x.key === key);
  if (!e) return;
  msg("⏳ Posting…", "#8a8d93");
  try {
    if (await postToDiscord(e)) msg("✅ Re-posted " + (e.ver || e.date) + " to #okanor-logs.");
  } catch (err) {
    msg("❌ " + err.message, "#ff6b6b");
  }
}
