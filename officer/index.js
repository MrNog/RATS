...
document.addEventListener("keydown", function (e) {
  if (e.key === "Escape") {
    document.getElementById("clog").classList.remove("open");
    document.getElementById("clogOv").classList.remove("open");
  }
});
...
(function loadChangelog() {
  ...
  fetch(FB, { cache: "no-store" })
    ...
    .then(function (o) {
      ...
      document.getElementById("clogEntries").innerHTML = list
        .map(function (e) {
          return (
            '<div class="cl-entry"><div class="cl-date">' +
            esc(fmtDate(e.date)) +
            (e.ver ? " · " + esc(e.ver) : "") +
            (e.pub ? " 🌐" : "") +
            "</div><ul>" +
