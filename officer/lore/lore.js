...
function populateHooks() {
  const sel = document.getElementById("hookSel"),
    hooks = loadHooks();
  if (!hooks.length) {
    sel.innerHTML = '<option value="">— no webhooks set (add in Admin) —</option>';
    return;
  }
  ...
  sel.onchange = () => {
    const h = hooks[+sel.value];
    if (h) localStorage.setItem("ratsLoreHook", h.name);
  };
}
function selectedHook() {
  const hooks = loadHooks();
  const i = +document.getElementById("hookSel").value;
  return hooks[i] || null;
}
...
function renderPreview() {
  ...
  let h = emojify(esc(raw))
    ...
    .replace(
      /&lt;@&amp;?(\d+)&gt;/g,
      '<span style="color:#c9b6f7;background:#3c3a5e;border-radius:3px;padding:0 3px">@mention</span>'
    ); // show pings as chips
...
function loadMentions() {
  try {
    const a = JSON.parse(localStorage.getItem("ratsMentions") || "null");
    return Array.isArray(a) ? a : [];
  } catch (e) {
    return [];
  }
}
function saveMentions(a) {
  localStorage.setItem("ratsMentions", JSON.stringify(a));
}
function mentionToken(m) {
  return m.role ? "<@&" + m.id + ">" : "<@" + m.id + ">";
}
function populateMentions() {
  const sel = document.getElementById("mentionSel"),
    list = loadMentions();
  sel.innerHTML = list.length
    ? list.map((m, i) => `<option value="${i}">${esc(m.label)}${m.role ? " (role)" : ""}</option>`).join("")
  ...
  box.innerHTML = list.length
    ? list
     ...
     <button class="alt" style="padding:4px 9px" onclick="removeMention(${i})">✕</button></div>`
        )
        .join("")
    : '<div class="sub" style="margin:0">No saved mentions yet.</div>';
}
function addMention() {
  ...
  if (!label || !/^\d{5,}$/.test(id)) {
    msg("❌ Enter a label and a numeric Discord ID.", "#ff6b6b");
    return;
  }
  const list = loadMentions();
  list.push({ label, id, role });
  saveMentions(list);
  document.getElementById("mLabel").value = "";
  document.getElementById("mId").value = "";
  populateMentions();
  msg("✅ Saved mention “" + label + "”.");
}
...
function toggleMentionMgr() {
  const e = document.getElementById("mentionMgr");
  e.style.display = e.style.display === "none" ? "" : "none";
}
function insertMention() {
  const list = loadMentions(),
    i = +document.getElementById("mentionSel").value;
  const m = list[i];
  if (!m) {
    msg("❌ No saved mention selected. Use ＋ Manage to add one.", "#ff6b6b");
    return;
  }
...
function addFiles(fl) {
  for (const f of fl) {
    if (loreFiles.length >= 10) break;
    if (f.type.startsWith("image/")) loreFiles.push(f);
  }
...
function setMType(b) {
  document.getElementById("mType").value = b.dataset.v;
  document.querySelectorAll("#mTypeSegs .seg").forEach((s) => s.classList.toggle("active", s === b));
}

async function postLore() {
  const hook = selectedHook();
  if (!hook || !WH_RE.test(hook.url)) {
    msg("❌ No valid webhook selected. Add one in the Admin console (named like “Lore”).", "#ff6b6b");
    return;
  }
  const raw = document.getElementById("body").value.trim();
  if (!raw && !loreFiles.length) {
    msg("❌ Write a story or add at least one image.", "#ff6b6b");
    return;
  }
  const content = emojify(raw); // shortcodes -> real emoji so Discord shows them
  if (content.length > 2000) {
    msg("❌ Discord limit is 2000 characters (you have " + content.length + ").", "#ff6b6b");
    return;
  }
  ...
  try {
    ...
    msg("❌ Post blocked (CORS / file://). Works on the hosted https site.", "#ff6b6b");
  }
}
