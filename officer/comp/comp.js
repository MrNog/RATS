...
const DEFAULT_CLASSES = [
  {
    ...
    specs: [
      { name: "Beastmastery", color: "#AAD372", emoteId: "637564202021814277" },
      { name: "Marksmanship", color: "#AAD372", emoteId: "637564202084466708" },
      { name: "Survival", color: "#AAD372", emoteId: "637564202130866186" },
    ],
  },
  {
    name: "Druid",
    ...
    specs: [
      { name: "Balance", color: "#FF7C0A", emoteId: "637564171994529798" },
      { name: "Feral", color: "#FF7C0A", emoteId: "637564172061900820" },
      { name: "Restoration", color: "#FF7C0A", emoteId: "637564172007112723" },
      { name: "Guardian", color: "#FF7C0A", emoteId: "637564171696734209" },
    ],
  },
  {
    name: "Warrior",
    ...
    specs: [
      { name: "Arms", color: "#C69B6D", emoteId: "637564445031399474" },
      { name: "Fury", color: "#C69B6D", emoteId: "637564445215948810" },
      { name: "Protection", color: "#C69B6D", emoteId: "637564444834136065" },
    ],
  },
  {
    name: "Priest",
    ...
    specs: [
      { name: "Discipline", color: "#FFFFFF", emoteId: "637564323442720768" },
      { name: "Holy", color: "#FFFFFF", emoteId: "637564323530539019" },
      { name: "Shadow", color: "#FFFFFF", emoteId: "637564323291725825" },
    ],
  },
  {
    name: "Mage",
    ...
    specs: [
      { name: "Arcane", color: "#3FC7EB", emoteId: "637564231545389056" },
      { name: "Fire", color: "#3FC7EB", emoteId: "637564231239073802" },
      { name: "Frost", color: "#3FC7EB", emoteId: "637564231469891594" },
    ],
  },
  {
    name: "Shaman",
    ...
    specs: [
      { name: "Elemental", color: "#0070DD", emoteId: "637564379595931649" },
      { name: "Enhancement", color: "#0070DD", emoteId: "637564379772223489" },
      { name: "Restoration1", color: "#0070DD", emoteId: "637564379847458846" },
    ],
  },
  {
    name: "DK",
    emoteId: "599012538935410701",
    specs: [
      { name: "Blood_DPS", color: "#C41E3A", emoteId: "1013371105874018405" },
      { name: "Frost_DPS", color: "#C41E3A", emoteId: "1013371107610468445" },
      { name: "Unholy_DPS", color: "#C41E3A", emoteId: "1013371108575162419" },
      { name: "Blood_Tank", color: "#C41E3A", emoteId: "1013371175210065960" },
      { name: "Frost_Tank", color: "#C41E3A", emoteId: "1013371176430620725" },
      { name: "Unholy_Tank", color: "#C41E3A", emoteId: "1013371178485817424" },
    ],
  },
  {
    name: "Paladin",
    ...
    specs: [
      { name: "Holy1", color: "#F48CBA", emoteId: "637564297622454272" },
      { name: "Protection1", color: "#F48CBA", emoteId: "637564297647489034" },
      { name: "Retribution", color: "#F48CBA", emoteId: "637564297953673216" },
    ],
  },
  {
    name: "Rogue",
    ...
    specs: [
      { name: "Assassination", color: "#FFF468", emoteId: "637564351707873324" },
      { name: "Combat", color: "#FFF468", emoteId: "637564352333086720" },
      { name: "Subtlety", color: "#FFF468", emoteId: "637564352169508892" },
    ],
  },
  {
    name: "Warlock",
    ...
    specs: [
      { name: "Affliction", color: "#8788EE", emoteId: "637564406984867861" },
      { name: "Demonology", color: "#8788EE", emoteId: "637564407001513984" },
      { name: "Destruction", color: "#8788EE", emoteId: "637564406682877964" },
    ],
  },
];
function baseComp() {
  return {
    title: "",
    groupCount: 5,
    groups: [1, 2, 3, 4, 5].map((i) => ({ name: "Group " + i, position: i })),
...
function realClassName(m) {
  const f = CLASSES.find((x) => x[0] === resolveClassKey(m));
...
function saveEmojiMap(map) {
  try {
    localStorage.setItem("ratsEmoji", JSON.stringify(map));
  } catch (e) {}
}
...
function sanitizeName(s) {
  ...
  return s.length >= 2 ? s.slice(0, 32) : "spec";
}
...
function populateGuildNames() {
  const dl = document.getElementById("guildNames");
  if (!dl) return;
  dl.innerHTML = guildRoster()
    .map((m) => `<option value="${(m.name || "").replace(/"/g, "")}">`)
    .join("");
}
// guess a spec from the guild note text (publicNote/officerNote)
const SPEC_ALIASES = {
  Beastmastery: ["beast", "bm"],
  Marksmanship: ["marks", "mm"],
  Survival: ["surv", "sv"],
  Balance: ["balance", "boomkin", "boom", "bala", "moonkin"],
  Feral: ["feral", "cat"],
  Restoration: ["resto", "rdudu", "rdru", "tree"],
  Guardian: ["guardian", "bear"],
  Arms: ["arms"],
  Fury: ["fury"],
  Protection: ["prot"],
  Discipline: ["disc", "disco", "disci"],
  Holy: ["holy"],
  Shadow: ["shadow", "spriest"],
  Arcane: ["arcane"],
  Fire: ["fire"],
  Frost: ["frost"],
  Elemental: ["ele", "elem", "elemental"],
  Enhancement: ["enh", "enhance", "spellhance"],
  Restoration1: ["resto", "rsham", "rshamy", "tree"],
  Blood_DPS: ["blood dps"],
  Frost_DPS: ["frost dk", "frost dps", "frost"],
  Unholy_DPS: ["unholy", "uh"],
  Blood_Tank: ["bdk", "blood dk", "blood tank", "btank", "blood"],
  Frost_Tank: ["frost tank"],
  Unholy_Tank: ["unholy tank"],
  Holy1: ["holy"],
  Protection1: ["prot"],
  Retribution: ["ret", "retri", "rett"],
  Assassination: ["assa", "sin", "mut"],
  Combat: ["combat"],
  Subtlety: ["sub", "subtlety"],
  Affliction: ["affli", "aff"],
  Demonology: ["demo", "demono"],
  Destruction: ["destro", "destruction"],
};
...
function guildAutofill() {
  ...
  if (ok) {
    ...
    const guess = guessSpecName(
      m,
      cs.find((x) => x.name === key)
    );
    const sp = document.getElementById("arSpec");
    if (sp) {
      if (guess) sp.value = guess;
      if (!sp.value && sp.options.length > 1) sp.selectedIndex = 1;
    }
  }
  const main = isAltG(m) ? mainOfG(m) : null;
  if (main && hint) {
    hint.style.display = "";
    hint.innerHTML = `↳ <b>${m.name}</b> is <b>${main}</b>'s alt — will be added as <b>${main}</b> (${m.class})`;
  }
}
...
function guildSuggest() {
  ...
  const inp = document.getElementById("arName"),
    box = document.getElementById("arSuggest");
...
function prettySpec(n) {
  return (n || "").replace(/1$/, "").replace(/_/g, " ");
}
function fillSpecs() {
  const cs = window.__classes && window.__classes.length ? window.__classes : DEFAULT_CLASSES;
...
function populateAddForm() {
  const cs = window.__classes && window.__classes.length ? window.__classes : DEFAULT_CLASSES;
  const cl = document.getElementById("arClass");
  if (!cl) return;
  const hint = document.getElementById("arHint");
  if (hint) hint.style.display = "none";
  cl.innerHTML = '<option value="" disabled selected>Class…</option>';
  cs.filter((c) => c.name !== "Tank").forEach((c) => {
    const o = document.createElement("option");
    o.value = c.name;
...
function addRaider() {
  ...
  if (!typed) {
    setMsg("❌ Enter a name to add.");
    return;
  }
  const name = resolveMain(typed); // alts count as their main
  const clName = document.getElementById("arClass").value,
    specName = document.getElementById("arSpec").value;
  if (!clName || !specName) {
    setMsg("❌ Pick a class and spec.");
    return;
  }
...
function addNoShow() {
  ...
  const typed = document.getElementById("arName").value.trim();
  if (!typed) {
    setMsg("❌ Enter a name to mark no-show.");
    return;
  }
  const name = resolveMain(typed); // alts count as their main
  const clName = document.getElementById("arClass").value,
  ...
  data.noshows.push({
    ...
    specEmoteId: s && s.emoteId,
    classEmoteId: c && c.emoteId,
    role: roleEmoji(specName, clName),
  });
  ta.value = JSON.stringify(data);
  document.getElementById("arName").value = "";
  render();
  setMsg(
    "✅ " + name + " marked no-show — " + prettySpec(specName) + " " + (clName === "DK" ? "Death Knight" : clName),
    "#7CFC8A"
  );
}

// ---- interactive board: move / no-show / remove (operate on the jsonIn data) ----
function groupOpts(cur, count) {
  let o = "";
  for (let i = 1; i <= count; i++) o += `<option value="${i}"${i === cur ? " selected" : ""}>G${i}</option>`;
  return o;
}
function _cd() {
  try {
    return JSON.parse(document.getElementById("jsonIn").value.trim());
  } catch (e) {
    return null;
  }
}
function _commit(d) {
  document.getElementById("jsonIn").value = JSON.stringify(d);
  render();
}

// ---- edit a raider's spec inline (fixes the spec/role + icon, recomputes tank/heal/dps) ----
function specClassObj(m) {
  return DEFAULT_CLASSES.find((x) => x.name.toLowerCase() === resolveClassKey(m)) || null;
}
function specOpts(m) {
  ...
  return c.specs
    .map(
      (s) =>
        `<option value="${esc(s.name)}"${s.name === m.specName ? " selected" : ""}>${esc(prettySpec(s.name))}</option>`
    )
    .join("");
}
function setSlotSpec(i, specName) {
  const d = _cd();
  if (!d || !d.slots || !d.slots[i]) return;
  const m = d.slots[i];
  const c = specClassObj(m);
  const s = c && c.specs.find((x) => x.name === specName);
  m.specName = specName;
  if (s) {
    m.specEmoteId = s.emoteId;
    m.color = s.color;
  }
  if (c) {
    m.classEmoteId = c.emoteId;
    if (/^(tank)?$/i.test(m.className || "")) m.className = c.name;
  }
  _commit(d);
}
...
function moveSlot(i, g) {
  const d = _cd();
  if (!d || !d.slots || !d.slots[i]) return;
  const m = d.slots.splice(i, 1)[0];
  m.groupNumber = parseInt(g) || 1;
  d.slots.push(m);
  renumber(d);
  _commit(d);
}
...
function onDropSlot(ev, targetIdx) {
  ...
  const d = _cd();
  if (!d || !d.slots) return;
  const target = d.slots[targetIdx];
  if (!target) return;
  ...
  if (p[0] === "slot") {
    ...
    moving = d.slots.splice(from, 1)[0];
  } else if (p[0] === "ns") {
    if (!d.noshows || !d.noshows[+p[1]]) return;
    moving = d.noshows.splice(+p[1], 1)[0];
  }
  if (!moving) return;
...
function slotNoShow(i) {
  ...
  d.noshows.push({
    ...
    specName: m.specName,
    color: m.color,
    specEmoteId: m.specEmoteId,
    classEmoteId: m.classEmoteId,
    role: roleEmoji(m.specName, m.className),
  });
  _commit(d);
}
function delNoShow(i) {
  const d = _cd();
  if (!d || !d.noshows || !d.noshows[i]) return;
  d.noshows.splice(i, 1);
  _commit(d);
}
function noShowToGroup(i, g) {
  const d = _cd();
  if (!d || !d.noshows || !d.noshows[i] || !g) return;
...
function render() {
  ...
  if (!rawJson) {
    data = { slots: [] };
  } // empty -> show a template (empty groups)
  else {
    try {
  ...
  document.getElementById("boardDesc").innerHTML =
    window.RatsData && RatsData.raidBadge ? RatsData.raidBadge(dIn) : esc(dIn || "");
  ...
  const groupCount =
    data.groupCount || (slots.length ? Math.max(...slots.map((s) => s.groupNumber || 1)) : templateGroups);
  window.__classes = data.classes || [];
  window.__groupCount = groupCount;
  const groupNames = {};
  (data.groups || []).forEach((g) => {
    if (g.position) groupNames[g.position] = g.name;
  });
  const byGroup = {};
  slots.forEach((s) => {
    (byGroup[s.groupNumber] = byGroup[s.groupNumber] || []).push(s);
  });
  ...
  for (let g = 1; g <= groupCount; g++) {
    ...
    members.forEach((m) => {
      total++;
      let role;
      if (tankSpecs.test(m.specName)) {
        tanks++;
        role = "🛡️";
      } else if (healSpecs.test(m.specName)) {
        heals++;
        role = "💚";
      } else {
        dps++;
      ...
      html +=
        `<div class="slot" draggable="true" ondragstart="onDragStart(event,'slot',${idx})" ondragend="dragEnd()" ondragover="allowDrop(event)" ondrop="onDropSlot(event,${idx})">${img}<span class="nm" style="color:${esc(m.color) || "#fff"}">${esc(m.name)}</span>` +
  ...
  nsArr.forEach((m, i) => {
    ...
    const img = icon
      ...
      : `<span class="ic"></span>`;
    html +=
      `<div class="slot" draggable="true" ondragstart="onDragStart(event,'ns',${i})" ondragend="dragEnd()" style="opacity:.85"><span class="ic" style="margin-right:-3px;text-align:center">❌</span>${img}<span class="nm" style="color:${esc(m.color) || "#fff"}">${esc(m.name)}</span>` +
      `<span class="acts"><button title="Remove" onclick="delNoShow(${i})">✕</button></span></div>`;
  });
  if (!nsArr.length)
    html += `<div class="sub" style="margin:6px 2px;font-style:italic">drop a raider here to mark a no-show</div>`;
  html += `</div>`;

  document.getElementById("grid").innerHTML = html;
  const nb = document.getElementById("noshowBox");
  if (nb) nb.innerHTML = "";
  window.__comp = comp;
...
(function () {
  let payload;
  try {
    payload = JSON.parse(localStorage.getItem("ratsCompEdit") || "null");
  } catch (e) {
    payload = null;
  }
  if (!payload) return;
  ...
  try {
    ...
    if (payload.date) {
      const di = document.getElementById("dateIn");
      di.value = payload.date;
      try {
        localStorage.setItem("ratsDate", payload.date);
      } catch (e) {}
      if (window.RatsCal) RatsCal.sync();
    }
    if (payload.size) {
...
function postWebhook() {
  ...
  if (!window.__comp || !window.__comp.length) {
    setMsg("❌ Import a comp first.");
    return;
  }
  if (((window.__stats && window.__stats.total) || 0) < MIN_RAIDERS) {
    setMsg("❌ Need at least " + MIN_RAIDERS + " raiders to post.");
    return;
  }
  ...
  const groupFields = window.__comp
  ...
  fetch(url, {
    ...
    .catch(() =>
      setMsg(
        "❌ Post blocked (CORS / file:// origin). Serve via a local server: python -m http.server, then open http://localhost:8000/officer/comp/"
      )
    );
}
...
async function saveToHistory() {
  ...
  if (!pass) {
    setMsg("❌ Locked — open the tools from the index and enter the guild key first.");
    return;
  }
  if (!window.__comp || !window.__comp.length) {
    setMsg("❌ Render a comp first (Import JSON or add raiders).");
    return;
  }
  if (((window.__stats && window.__stats.total) || 0) < MIN_RAIDERS) {
    setMsg("❌ Need at least " + MIN_RAIDERS + " raiders to save.");
    return;
  }
  const dateVal = document.getElementById("dateIn").value;
  if (!dateVal) {
    setMsg("❌ Pick a raid date first (the calendar at the top).");
    return;
  }
  ...
  try {
    ...
    const entry = {
      ...
      noshows: (window.__noshows || []).map((m) => ({
        name: m.name,
        className: m.className,
        specName: m.specName,
        specEmoteId: m.specEmoteId,
    ...
    setMsg(
      "✅ " +
        ...
        " raiders)" +
        (res.mode === "firebase"
          ? " — saved & shared instantly. 🐀"
          : " — commit the downloaded history.json via Fork to share it."),
      "#7CFC8A"
    );
  } catch (e) {
    setMsg("❌ Save failed: " + (e && e.message ? e.message : e), "#ff6b6b");
  }
}
...
function openRosterPicker() {
  if (!guildRoster().length) {
    setMsg("❌ No roster loaded — open the tools from the index (enter the guild key) first.");
    return;
  }
  rpSelected = new Set();
...
function updateRpCount() {
  const e = document.getElementById("rpCount");
  if (e) e.textContent = rpSelected.size + " selected";
}
function rpToggle(cb) {
  const k = (cb.getAttribute("data-name") || "").toLowerCase();
  if (cb.checked) rpSelected.add(k);
  else rpSelected.delete(k);
  updateRpCount();
}
function rpSelectAll(v) {
  document.querySelectorAll("#rosterList input[type=checkbox]").forEach((cb) => {
    cb.checked = v;
    const k = (cb.getAttribute("data-name") || "").toLowerCase();
    if (v) rpSelected.add(k);
    else rpSelected.delete(k);
  });
  updateRpCount();
}
function renderRosterPicker() {
  ...
  if (q)
    roster = roster.filter(
      (m) => (m.name || "").toLowerCase().includes(q) || (m.class || "").toLowerCase().includes(q)
    );
  if (!roster.length) {
    box.innerHTML = '<div class="sub" style="padding:10px;margin:0">No roster loaded, or nothing matches.</div>';
    updateRpCount();
    return;
  }
  const order = [
  ...
  const byClass = {};
  roster.forEach((m) => {
    (byClass[m.class] = byClass[m.class] || []).push(m);
  });
  ...
  classes.forEach((cl) => {
    ...
    members.forEach((m) => {
      ...
      const altTag = isAltG(m)
        ? ` <span style="color:#7aa2c9;font-style:italic;font-size:11px">↳${esc(mainOfG(m) || "")}</span>`
        : "";
...
function applyRosterPicker() {
  ...
  guildRoster().forEach((m) => {
    ...
    let specName = guessSpecName(m, c);
    if (!specName && c.specs.length) specName = c.specs[0].name;
  ...
  setMsg(
    "✅ Added " +
      ...
      (added != 1 ? "s" : "") +
      " from the roster" +
      (skipped ? " (" + skipped + " skipped — already in comp)" : "") +
      ".",
    "#7CFC8A"
  );
}
